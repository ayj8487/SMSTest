...
public class ExampleController {
	...
	final DefaultMessageService messageService;
	   ...
	   public ExampleController() {
	    ...
	    private String ranCode; // 전송된 인증번호 임시 저장
	    
		/*
		 * @PostMapping("/valiCode") public String valiCode(String inputCode, Model
		 * model ) { SingleMessageSentResponse ck = sendOne();
		 * 
		 * if(inputCode.equals(conCode)) {
		 * 
		 * 
		 * model.addAttribute("username", "안영준");
		 * model.addAttribute("email","ayj8487@naver.com");
		 * 
		 * String phone = "010-1234-5512"; // 중간번호 가리기 테스트 String maskPhon =
		 * maskPhon(phone); model.addAttribute("phone", maskPhon);
		 * 
		 * return "myCon"; // 사용자 정보 조회 페이지
		 * 
		 * } else { model.addAttribute("error", "인증번호가 일지하지 않습니다.") ; return "valiForm";
		 * }
		 * 
		 * }
		 */	    
	    ...
	    // 핸드폰 번호 가리기
	    public String maskPhon(String phone) {
	    	// 핸드폰 번호 중간 4 자리를 * 로 대체 
	    	String [] parts = phone.split("-"); // 핸드폰 번호 - 를 기준으로 분리
	    	if(parts.length == 3) { // 길이가 3인 배열인지 확인 010-1111-1111 처럼 되어있는지 형식 확인
	    	
	    		String mask =  parts[1].replaceAll(".", "*"); // 번호의 중간 부분을 * 로 대체
		 ...
		 * message.setFrom("01024846106"); // 발신번호 입력 message.setTo("01024846106"); //
		 * 수신번호 입력 // 한글 45자, 영자 90자 이하 입력되면 자동으로 SMS타입의 메시지가 추가됩니다.
		 * message.setText("[BookMaket 우주최강3조 최공] ["+ranCode+"] 인증번호를 정확히 입력해주세요.");
		 * 
	    ...
	    public String sendOne(String inputCode, Model model ) {
	        Message message = new Message();
